...
ENABLE_INGRESS_FILTER = os.environ.get("REMATTERS_INGRESS_FILTER", "0").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
...
app = FastAPI(title="Rematters", version="0.1.28-docker", lifespan=lifespan)
...
async def ingress_ip_filter(request: Request, call_next):
    """Optional Supervisor ingress filter (disabled by default in Docker)."""
    if not ENABLE_INGRESS_FILTER:
...
async def cloud_status():
    ...
    return {
        ...
        if cloud_configured()
            ...
            "Set cloud_url and cloud_token in options.json."
            if not (has_url and has_token)
            else "Enable cloud_enabled in options.json."
...
async def cloud_sync_now():
    if not cloud_configured():
        ...
        detail = status.get("hint") or "Configure Rematters Cloud in options.json"
...
async def index():
    ...
    return JSONResponse({"service": "rematters", "version": "0.1.28-docker"})
